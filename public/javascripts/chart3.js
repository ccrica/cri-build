...
const ctxB = document.getElementById('myBars');
...
//                backgroundColor: [cVide, legendLabelColors[0], cVide, legendLabelColors[1],cVide,  legendLabelColors[2],cVide,  legendLabelColors[3],cVide, legendLabelColors[4],cVide, legendLabelColors[5]],
    ...
    options: {
        ...
        animation: {
          onComplete: function() {
              drawLogo2();
              console.log('1. Fonction logo appelée depuis le chart  sur chart3')
          }
      }
    }
}

const Chart3 = new Chart(ctxB, config);


function drawLogo2() {
  console.log('2. Fonction logo appelée sur chart3');
  const img = new Image();
  img.onload = function() {
    const ctxC = ctxB.getContext('2d');
      // Get the position and size of the chart
      const chartX = Chart3.chartArea.left;
      const chartY = Chart3.chartArea.top;
      const chartWidth = Chart3.chartArea.right - Chart3.chartArea.left;
      const chartHeight = Chart3.chartArea.bottom - Chart3.chartArea.top;
